...
class _MainScreenState extends State<MainScreen> {
   final GlobalKey<ScaffoldState> _scaffoldKey = GlobalKey<ScaffoldState>();
    ...
    void _shareNote(List<Note> notes) async {
      ...
      final isBulk = notes.length > 1;
      final title = isBulk ? 'Share ${notes.length} Notes' : 'Share Note: ${notes.first.title}';

      // Load all notes including secret
      final allNotes = await _noteService.loadAllNotes();
      final shareNotes = allNotes.where((note) => notes.any((n) => n.id == note.id)).toList();

      // Get local IP
      final interfaces = await NetworkInterface.list();
      String ip;
      try {
        ip = interfaces.firstWhere(
          (iface) => iface.name.contains('wlan') || iface.name.contains('eth') || iface.name.contains('Wi-Fi') || iface.name.contains('Ethernet'),
          orElse: () => interfaces.firstWhere((iface) => iface.addresses.isNotEmpty),
      ...
      final handler = shelf.Pipeline().addHandler((shelf.Request request) {
        ...
        if (request.url.path == '/notes' || request.url.path == 'notes') {
          final notesData =
              jsonEncode(shareNotes.map((n) => n.toMap()).toList());
      ...
      try {
        ...
        server = await io.serve(handler, ip, 0); // Port 0 auto-assigns
      }
      print('Server running on http://${server.address.host}:${server.port}');

      final networkData = jsonEncode({'ip': server.address.host, 'port': server.port});
      showDialog(
        context: context,
    ...
    void _processScannedData(String data) async {
      ...
      try {
        ...
        if (ip != null && port != null) {
          ...
          try {
            final response = await http.get(Uri.parse('http://$ip:$port/notes')).timeout(const Duration(seconds: 10));
            if (response.statusCode == 200) {
              ...
              if (mounted) {
                ScaffoldMessenger.of(context).showSnackBar(
                  SnackBar(content: Text('Server responded with error: ${response.statusCode}')),
                );
              }
            }
          } catch (e) {
    ...
    void _handleReceivedNotes(List<Note> notes) async {
      ...
      if (hasSecret) {
        ...
        if (pin != null && pin.isNotEmpty) {
          ...
          if (mounted) {
            ScaffoldMessenger.of(context).showSnackBar(
              SnackBar(content: Text('${notes.length} notes imported successfully')),
            );
          }
        }
      } else {
    ...
    void _navigateToSecretNotes() async {
     ...
     if (pinResult is String && mounted) {
        Navigator.push(
          context,
          MaterialPageRoute(
            builder: (context) => SecretNoteListPage(pin: pinResult, onShareNote: _shareNote),
          ),
        );
     }
   }
